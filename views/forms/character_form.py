...
class CharacterForm(QWidget, DottedBackgroundMixin):
    saved = pyqtSignal(dict)

    def __init__(self, parent=None, scenario_id=None, character=None):
        ...
        self.setWindowTitle("Редактировать персонажа" if character else "Новый персонаж")
        ...
        self.le_role.setMinimumHeight(30)
        role_layout.addWidget(self.le_role)
        ...
        # Заполняем поля, если передан персонаж для редактирования
        if character:
            self.le_name.setText(character.name)
            self.le_role.setText(character.role or "")
            self.txt_desc.setPlainText(character.description or "")
            self.txt_note.setPlainText(character.note or "")

    def _on_save(self):
        data = {
            'name': self.le_name.text().strip(),
            'role': self.le_role.text().strip() or None,
